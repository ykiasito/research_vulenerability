...
以上の1〜6(とCVE.orgについての判断)が一度完了して初めて、**このアプリが実際に照合できるエコシステムについては**ジョブ結果を信用してよい状態になります——この限定が必要な理由は、すぐ下の「このアプリで照合できないもの」を参照してください(データをどれだけ投入してもこの制約は変わりません)。ここでネットワークを閉じて実運用に入ってよい状態にもなります(公開範囲をこれ以上広げる段階ではありません)。

### このアプリで照合できないもの(データをいくら投入しても変わらない)

一部のソフトウェアは、このブランチには自動照合の経路自体がありません——上記の手順をどれだけ完璧にこなしても関係ありません。アプリ内のガイド(`/guide/integrations`)の3章「自動照合できない配布チャネル」に明記されており、現時点では以下の通りです。

- Maven Central(Java、`group:artifact:version` 形式の座標)
- Docker Hub(コンテナイメージ)
- apt/rpm等、Linuxディストリビューションの各パッケージマネージャ
- VS Code Marketplace(拡張機能)
- Chrome ウェブストア(拡張機能)
- Homebrew(macOS/Linuxパッケージ)
- 各種App Store / モバイルアプリストア

**Maven Centralについては特に注意**: 上記の手順6で扱った9レジストリと異なり、このブランチにはMaven Centralのミラーが無く、ライブ照会によるフォールバックもありません(`MavenCentralRegistryClient#lookup` は常に `Optional.empty()` を返す設計——クラスのjavadoc参照)。これはシード登録や同期では解決できない制約です——Maven座標の行は、このブランチでは常に「該当なし」としか返ってきません。上記のいずれかに該当する製品は、別の手段(ベンダー自身のアドバイザリーを直接確認する等)で調査してください——これらの行が「クリーン」に見えても「既知の脆弱性なし」とは解釈しないでください。
