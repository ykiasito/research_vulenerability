...
同期処理はいずれもCSVアップロードそのものを直接のトリガーにはしませんが、自動実行される契機・既定の有効/無効は同期対象によって異なります。ひとまとめに「全て admin opt-in」とは言えないため、以下のように区別します。
...
### 環境変数によるゲートを持たず、起動しただけで自動実行されるもの

以下5つの日次デルタ同期は、有効化フラグ自体が存在しない無条件の`@Scheduled`ジョブです。既定設定のまま起動しただけで、管理者が何も設定しなくても毎日自動的に外部へHTTPリクエストが発生します(いずれも増分[delta]取得のみで、フルベースラインの再取得は`AdminController`経由の手動トリガーのみ)。

- GHSA(`GhsaScheduledSync` → `GhsaSyncService`)
- OSV(`OsvScheduledSync` → `OsvSyncService`)
- CVE.org(`CveOrgScheduledSync` → `CveOrgSyncService`)
- CSAF Red Hat(`RedHatCsafScheduledSync` → `RedHatCsafSyncService`)
- CSAF Siemens(`SiemensCsafScheduledSync` → `SiemensCsafSyncService`)

これらはいずれも公開されている脆弱性アドバイザリの全件差分取得であり、特定のCSVアップロード内容とは無関係です。ただし「同期は全て管理者のopt-inでのみ動く」わけではなく、この5つは既定のまま自動的に動作する点に注意してください。
...
一方で、GHSA/OSV/CVE.org/CSAF(Red Hat・Siemens)の日次デルタ同期は既定設定のまま起動しただけで自動的に外部へHTTPリクエストを送信します(公開アドバイザリの全件差分取得であり、個々のCSVアップロード内容とは無関係)。また、レジストリミラー同期(既定無効)を明示的に有効化した場合は、過去のアップロード内容が識別処理を経て解決したパッケージ名が外部レジストリへの問い合わせ対象になり得ます。これらはいずれも「CSVをアップロードして識別処理を実行する」という操作そのものが引き起こす通信ではありません。
...
- **アプリケーションログでの確認**: 同期系コンポーネントはそれぞれ固有のUser-Agent文字列(例: `vulncheck-server/0.1 (cpe dictionary sync)` のような、コンポーネントごとに異なる識別子)を使ってHTTPリクエストを送るため、識別処理を実行した時間帯のログにこれらの文字列が一切出現しないことを確認する。GHSA/OSV/CVE.org/CSAF(Red Hat・Siemens)の日次デルタ同期には有効化フラグ自体が存在しないため、これらについてはこのログ確認が主な検証手段になる。
- **設定値の確認**: `cpe-scheduled-resync-enabled`・`registry-mirror-scheduled-sync-enabled`・`nvd-cve-backfill.enabled`(いずれも既定`false`)が意図せず有効化されていないことを確認する。この3つ以外の同期(GHSA/OSV/CVE.org/CSAF)には対応するフラグが無く、既定のまま自動的に動作するため、これらは設定値の確認では検知できない(上のログ観測、またはネットワークトラフィックの直接観測で確認する)。
