...
> 本ディレクトリ（`docs/public/`）は、アーキテクチャ・パイプライン・スキーマの技術リファレンスに限定して公開している。タスク管理・稼働ログ・進捗計画・内部運用の詳細等は対象外。会社名や個人情報はそもそも本ドキュメントに記載していない。エージェント運用体制の語彙やAPIクレジット残高の状況等、内部運用の生々しい詳細については一般化している。
