...
public class JobController {
    ...
    public String detail(@AuthenticationPrincipal UserDetails userDetails, @PathVariable Long id,
        ...
        // Closed-mode backlog item 382 (promoted to master by item 396): a stale/never-baselined
        // mirror silently degrades every result on this page with no other on-page signal that
        // anything is wrong -- see MirrorFreshnessService's own class javadoc for what "stale" means
        // per mirror, and for which of its checks apply unconditionally vs. only when gated on.
