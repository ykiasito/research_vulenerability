 ...
 * behavior changes, not a compliance mechanism for a known limit — same rationale tier as {@link
 * OsvRateLimiter}'s own javadoc for {@code api.osv.dev}.
 *
 ...
 * no meaningful effect on sync completion time. 200ms was picked as a middle ground between {@link
 * OsvRateLimiter}'s 100ms (a documented-no-limit, actively-queried-per-item endpoint) and {@link
 * GhsaRateLimiter}'s 65s (a hard published cap) — plan §10-1 item 4 leaves the exact figure as an
