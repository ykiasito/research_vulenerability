...
public class CveOrgSyncService {
    ...
    public int syncBaseline() {
        ...
        try (InputStream outerStream = download(release.baselineZipUrl());
            ...
            // Closed-mode backlog item 379: record the failure (rather than silently returning)
            // so /admin/cve-org and the mirror-freshness check (item 382) both have a signal a
            // continuously-failing baseline sync used to leave nowhere — deliberately not
            // e.getMessage() (this class's own convention elsewhere, see #download's javadoc):
            // an IOException here could in principle still be wrapping something that read part
            // of the signed download URL before failing.
     ...
     *  failure is visible on /admin/cve-org and to the mirror-freshness check (item 382) instead of
     *  vanishing into the log alone. Deliberately still advances {@code last_synced_at} to "now" —
     *  matching {@code GhsaSyncService#failSync}'s convention — so this state's last_synced_at means
     *  "the last time a sync was attempted", not "the last time one succeeded"; {@code
     *  last_sync_error} being non-null is what actually distinguishes the two, which is why the
     *  mirror-freshness check must look at both fields, not last_synced_at alone. Never touches
     *  {@code baseline_loaded}/{@code last_release_tag} — a failed attempt must not make a
     *  previously-completed baseline look un-loaded, nor overwrite a known-good release tag with
     *  nothing. */
