...
class ReverifyJobCreator {
    ...
    @Test
    void createReverifyJob() throws Exception {
        try (InputStream csv = getClass().getClassLoader().getResourceAsStream("reverify.csv")) {
            ResearchJob job = researchJobService.createJob(
                    REAL_USER_ID, "reverify.csv", csv, ColumnMapping.identity(), false);
            System.out.println("\n=== CREATED REVERIFY JOB ID: " + job.getId() + " ===\n");
        }
    }
}
