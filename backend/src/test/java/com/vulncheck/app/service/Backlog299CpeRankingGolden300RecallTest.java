...
@Disabled("Re-measured 2026-09-05 (backlog item 346, max_cataloged_major outlier guard) against the "
        + "real dev DB, before vs. after item 346's rule A/B outlier guard -- CPE vendor:product exact "
        + "match is unchanged at 64/68 = 0.9412 either way, confirming item 346 is neutral for this "
        + "golden-300 subset (the 4 remaining mismatches -- Metasploit Framework, Notepad++, Zoom, "
        + "Kibana -- are all unrelated to max_cataloged_major). Left disabled so it can never re-fire "
        + "on a routine mvn test run -- see class javadoc. The max cataloged major for "
        + "oracle:vm_virtualbox used to read 71, traced to a single broken NVD version string '71.6' "
        + "among that product's 270 rows (a typo, not a real major version 71 release) -- backlog item "
        + "346's outlier guard in CpeDictionaryRepositoryImpl#collect now corrects this to 7 (the real "
        + "highest cataloged major), which is still <= VirtualBox 7.0.14's own major, so "
        + "versionCoverageRank's outcome for this row is unchanged by item 346.")
