...
class Stage1IdentificationServiceTest {
    ...
    void mavenArtifactTokenVariantCandidateIsDroppedRatherThanAutoAcceptedWithNoApiKey() {
        // Backlog item 388 (REVISE, senior review 2026-09-06): the Maven-coordinate artifactId-token
        // variant fallback is subject to the same forced-AI-check as any other variant-derived
        // candidate (see aLoneNameVariantDerivedCpeCandidateIsDroppedRatherThanAutoAcceptedWithNoApiKey
        // above) — closed mode's Stage1AiArbitration always returns Optional.empty() (no Claude key
        // is ever configured in this mode), so a mechanically-derived artifactId-token guess like
        // this one is always dropped rather than auto-accepted.
    ...
    void mavenArtifactTokenVariantFallbackNeverFiresWhenARegistryMatchAlreadyCoversTheItem() {
        ...
        // token-variant fallback and the exact-vendor-product fallback must both be structurally
        // unreachable once a registry match already covers the item.
        verify(cpeDictionaryRepository, org.mockito.Mockito.times(1))
                .findFuzzyMatches(anyString(), anyDouble(), anyDouble(), anyInt());
    }
