...
@Disabled("Item 303 (task B, install_url-hostname-driven passesTargetSwGate generalization) "
        + "regression re-check (2026-09-05) against the real dev DB: recall 265/266=0.9962, "
        + "control-row false-positive rate 3/34=0.0882 -- both same-or-better than the prior "
        + "re-measurement below (264/266, 5/34). golden-300.csv has zero non-blank install_url "
        + "rows, so item 303's own code change (a strictly additive install_url-derived "
        + "declaredTargetSw source) is a structural no-op here -- every item still resolves through "
        + "exactly the same registry-ecosystem-or-none gating path as before. The improvement seen "
        + "is attributable to other already-merged work on this branch (item 302's (vendor, product) "
        + "exact-match fallback), not to this item, and is recorded here only to confirm no "
        + "regression. Run once (2026-08-31, backlog item 99 Chocolatey-removal recall "
        + "re-measurement) against "
